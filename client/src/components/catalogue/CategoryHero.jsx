...
export default function CategoryHero({ title, description, categoryName, categoryDescription }) {
  ...
  return (
    <section className="category-hero fade-in">
      ...
      <style>{`
        .category-hero { min-height: clamp(280px, 45vh, 460px); display: flex; align-items: end; padding: 126px 0 56px; background: linear-gradient(135deg, #ffffff 0%, var(--cream) 58%, #dff0ff 100%); color: var(--ink); border-bottom: 1px solid var(--border); }
        .category-hero h1 { margin: 0 0 12px; color: var(--ink); text-transform: capitalize; }
        .category-hero p { max-width: 620px; margin: 0; color: var(--ink-soft); line-height: 1.7; }
