...
export default function Hero() {
  return (
    <header className="hero" id="home">
      ...
      <style>{`
        .hero { position: relative; min-height: 100vh; padding: 126px clamp(24px, 5vw, 80px) 64px; background: #0b1120; color: var(--white); overflow: hidden; scroll-margin-top: 90px; }
        .hero::before { content: ''; position: absolute; inset: 0; background-image: linear-gradient(90deg, rgba(9,12,20,0.92) 0%, rgba(9,12,20,0.74) 45%, rgba(9,12,20,0.28) 100%), url('https://images.unsplash.com/photo-1441986300917-64674bd600d8?auto=format&fit=crop&w=1800&q=80'); background-position: center; background-size: cover; transform: scale(1.04); animation: heroImageDrift 16s var(--ease-out) infinite alternate; pointer-events: none; }
        .hero::after { content: ''; position: absolute; inset: 0; background-image: linear-gradient(180deg, rgba(11,17,32,0) 76%, #090c14 100%), radial-gradient(circle at 18% 22%, rgba(200,169,110,0.18), transparent 28%); pointer-events: none; }
        ...
        .hero h1 { margin: 0 0 24px; max-width: 900px; color: var(--white); font-size: clamp(2.9rem, 6.6vw, 5.8rem); line-height: 0.98; letter-spacing: 0; }
        .hero-copy p { margin: 0 0 34px; max-width: 690px; color: rgba(255,255,255,0.76); font-size: clamp(1rem, 1.4vw, 1.16rem); line-height: 1.8; }
        ...
        .btn:hover { transform: translateY(-2px); box-shadow: 0 18px 50px rgba(0, 0, 0, 0.22); }
        .btn-primary { background: var(--white); color: var(--ink); }
        .btn-secondary { background: rgba(255,255,255,0.08); color: var(--white); border-color: rgba(200,169,110,0.72); backdrop-filter: blur(10px); }
